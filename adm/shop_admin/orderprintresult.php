...
if ($csv == 'csv')
{
    ...
    $sql = " SELECT a.od_id, od_b_zip1, od_b_zip2, od_b_addr1, od_b_addr2, od_b_addr3, od_b_addr_jibeon, od_b_name_last, od_b_name, od_b_tel, od_b_hp, b.it_name, ct_qty, b.it_id, a.od_id, od_memo, od_invoice, b.ct_option, b.ct_send_cost, b.it_sc_type
    ...
    for ($i=0; $row=mysql_fetch_array($result); $i++)
    {
        ...
        echo '"'.$row['od_name_last'].$row['od_b_name'].'"'.',';
...
if ($csv == 'xls')
{
    ...
    for($i=1; $row=sql_fetch_array($result); $i++)
    {
        ...
        $worksheet->write($i, 2, $row['od_b_name_last'].$row['od_b_name']);
...
<div id="sodr_print_pop" class="new_win">
    ...
    <div class="sodr_print_pop_list">
        ...
        <h3>보내는 사람 : <?php echo get_text($row1['od_name_last']); ?><?php echo get_text($row1['od_name']); ?></h3>
        ...
        <?php } else { ?>
        <h3>받는 사람 : <?php echo get_text($row1['od_b_name_last']); ?><?php echo get_text($row1['od_b_name']); ?></h3>
        <dl>
            <dt>주소</dt>
