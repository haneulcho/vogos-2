...
function valid_mb_id($reg_mb_id)
{
    if (preg_match("/[^0-9a-z_]+/i", $reg_mb_id))
        return "You can use ID only A-Z, 0-9. Please try again.";
    else
        return "";
}
...
function count_mb_id($reg_mb_id)
{
    if (strlen($reg_mb_id) < 3)
        return "Please enter your ID at least 3 characters.";
    else
        return "";
}
...
function exist_mb_id($reg_mb_id)
{
    ...
    $row = sql_fetch($sql);
    if ($row['cnt'])
        return "Your ID already exists, Please try again.";
    else
        return "";
}
...
function valid_mb_email($reg_mb_email)
{
    if (!preg_match("/([0-9a-zA-Z_-]+)@([0-9a-zA-Z_-]+)\.([0-9a-zA-Z_-]+)/", $reg_mb_email))
        return "E-mail address is invalid. please try again.";
    else
        return "";
}
...
function exist_mb_email($reg_mb_email, $reg_mb_id)
{
    ...
    if ($row['cnt'])
        return "Your E-mail address already exists, Please try again.";
    else
        return "";
}
...
function valid_mb_name($mb_name)
{
    if (!check_string($mb_name, G5_HANGUL))
        return "Your first name is invalid, Remove blank and try again.";
    else
        return "";
}
...
function exist_mb_hp($reg_mb_hp, $reg_mb_id)
{
    ...
    if($row['cnt'])
        return " Your mobile number is already exists. ".$reg_mb_hp;
    else
        return "";
}
